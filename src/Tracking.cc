...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Atlas *pAtlas, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor, const string &_nameSeq, bool bReuseMap):
...
//    if(mState!=NO_IMAGES_YET)
//    {
//         //XLOG_DEBUG("this is 0, 2+ key frames");
//        // 判断时间上的前后关系
//        // 1是时间上不能反 2是间隔不能超过1s 有这种情况直接重置地图
...
////                    {
////                        mpSystem->ResetActiveMap();   // 此时 mState = NO_IMAGES_YET;
////                    }
////                    else
////                    {
////                        CreateMapInAtlas();
...
//    }
    ...
    if(mState==NO_IMAGES_YET)
    {
        ...
        //XLOG_DEBUG("this is 1, first time not image yet");
    }
    ...
    mbCreatedMap = false;
    //XLOG_DEBUG("this is 2");
    //unsigned long int cur_frame_id = mCurrentFrame.mnId;
...
//--------------------------------跟踪开始---------------------------------------------------------------------
    ...
    if(mState==NOT_INITIALIZED)
    {
        ...
        else
        {
            ...
            //XLOG_DEBUG("this is 3, MonocularInitialization");
        }
    ...
    else // 初始化完成 开始追踪
    {
        ...
        if(!mbOnlyTracking) //SLAM正常模式
        {
            //XLOG_DEBUG("this is 4, normal SLAM mode");
            ...
            /// State != ok -------------- RECENTLY_LOST和LOST情况处理
            ...
            else
            {
                ...
                {
                    //XLOG_DEBUG("this is 11");
                    Verbose::PrintMess("A new map is started...", Verbose::VERBOSITY_NORMAL);
                    // 如果关键帧小于10 重置地图 否则创建一个新地图
        ...
        else
        {
            ...
            if(mState==LOST)
            {
                ...
                bOK = Relocalization();
                //XLOG_DEBUG("this is 12");
            }
            else
            {
                // mbVO是mbOnlyTracking为true时的才有的一个变量, 初始为false
                ...
                if(!mbVO)
                {
                    ...
                    {
                        bOK = TrackWithMotionModel();
                        //XLOG_DEBUG("this is 13,TrackWithMotionModel");
                    }
                    else
                    {
                        bOK = TrackReferenceKeyFrame();
                        //XLOG_DEBUG("this is 14,TrackReferenceKeyFrame");
                    }
                }
                // mbVO=true 快跪了的情况 没有跟踪上
                else
                {
                    ...
                    //XLOG_DEBUG("this is 15,Relocalization for 2 time");

                    if(bOKMM && !bOKReloc) // 恒速模型又跟踪上了
                    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            {
                bOK = TrackLocalMap();
                //XLOG_DEBUG("this is 16");

            }
            if(!bOK)
                cout << "Fail to track local map!" << endl;
        }
        else // 纯定位模式下TrackLocalMap
        {
            ...
            if(bOK && !mbVO)
                bOK = TrackLocalMap();
                //XLOG_DEBUG("this is 17,TrackLocalMap");
        }

        if(bOK)
            mState = OK;
        // 局部地区跟踪失败 但是之前的跟踪成功 imu为RECENT_LOST 纯视觉为LOST
        else if (mState == OK)
        {
            if (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO)
            {
                ...
                mState = LOST; // visual to lost 应该是源码bug
                //XLOG_DEBUG("this is 18");
        ...
        // TODO 可能要改代码
        // Step 5：跟踪成功，更新恒速运动模型，计算上一帧到当前帧的位姿变换
        ...
        if(mState==LOST)
        {
            //XLOG_DEBUG("this is 19");
            if(pCurrentMap->KeyFramesInMap()<=5)
            {
                mpSystem->ResetActiveMap();
                //XLOG_DEBUG("this is ResetActiveMap");
                return;
            }
            if ((mSensor == System::IMU_MONOCULAR) || (mSensor == System::IMU_STEREO))
                if (!pCurrentMap->isImuInitialized())
    ...
    if(mState==OK || mState==RECENTLY_LOST)
    {
        ...
        else
        {
            ...
            mlbLost.push_back(mState==LOST);
            //XLOG_DEBUG("this is 22");
        }

    }
//    XLOG_DEBUG("Key Frames conntected frames number: {}", mpLastKeyFrame->GetConnectedKeyFrames().size());
    //XLOG_DEBUG("this is end process of this frame track, mState:=== {}",mState);
...
void Tracking::MonocularInitialization()
{
    ...
    else // 第二帧
    {
        ...
        // 通过H模型或F模型进行单目初始化，得到两帧间相对运动、初始MapPoints bool
...
//        cv::Mat worldPos = mSim3.rowRange(0,3).colRange(0,3) * Pos + mSim3.rowRange(0,3).col(3);
...
//    pKFini->SetPose(cv::Mat::eye(4, 4, CV_32F) * Tlc); // 后加的
...
//    Tc2w = Tc2w * Tlc;
    ...
    for(size_t iMP=0; iMP<vpAllMapPoints.size(); iMP++)
    {
        if(vpAllMapPoints[iMP])
        {
            ...
            cv::Mat Pos_tem = pMP->GetWorldPos()*invMedianDepth;
//            pMP->SetWorldPos(mSim3.rowRange(0,3).colRange(0,3) * Pos_tem + mSim3.rowRange(0,3).col(3));
            pMP->SetWorldPos(Pos_tem);
            pMP->UpdateNormalAndDepth();
        }
    }
...
{   //XLOG_DEBUG("this is bebug by TrackWithMotionModel()");
...
void Tracking::UpdateLastFrame()
{
    ...
    // Tlw = Tlr*Trw
//    mLastFrame.SetPose(Tlr*pRef->GetPose() * Tlc);
...
bool Tracking::TrackWithMotionModel()
{
    ...
    //XLOG_DEBUG("this is bebug by TrackWithMotionModel() end");
}
/**
...
{
    //XLOG_DEBUG("this is debug by Relocalization()");
    ...
    cout << "Relocalization Candidate size: " << vpCandidateKFs.size() << endl;
    //XLOG_DEBUG("there are: {} candidates KFs", vpCandidateKFs.size());
    ...
    if(!bMatch)
    {
//        cout << "NOT Relocalized!!" << endl;
        //XLOG_DEBUG("this is debug by not relocalized!");
        return false;
    }
    else
    ...
    //XLOG_DEBUG("this is debug by Relocalization() end");
}
