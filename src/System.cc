    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false)
{
    ...
    else
    {
        ...
        //clock_t start = clock();
        bool isRead = LoadAtlas(strLoadingFile,1, strSettingsFile);
...
//        cout << "是否开启纯定位模式？(y/n) LocalizationMode?(Y/y)"<<endl;
...
//        }
        ...
        mpAtlas->PostLoad(strSettingsFile);
...
//        mpAtlas->CreateNewMap();
    ...
    mpTracker = new Tracking(this, mpVocabulary, mpFrameDrawer, mpMapDrawer,
                             mpAtlas, mpKeyFrameDatabase, strSettingsFile, mSensor, strSequence, loadedAtlas);
...
bool System::LoadAtlas(string filename, int type, string strSettingPath)
{
    bool isRead = false;
...
//                cout << "binary_iarchive error." << endl;
//                ifs.close();
//                return false;
//            }
        ...
        int iN = mpAtlas->CountMaps();
        cout<<"iN = "<<iN<<endl;
        for(int i = 0; i < iN; i++){
            vector<Map*> pMap = mpAtlas->GetAllMaps();
            mpAtlas->mpCurrentMap = pMap[iN-1];
            cout << "mTmp" << mpAtlas->mpCurrentMap->GetAllKeyFrames().size() << endl;
        }
...
//        ifs.close();
    ...
    cout<<"numbers of KFs:"<<mpAtlas->GetAllKeyFrames().size()<<endl;
    cout<<"numbers of MPs:"<<mpAtlas->GetAllMapPoints().size()<<endl;
    cout<<"numbers of MAPs:"<<mpAtlas->GetAllMaps().size()<<endl;
