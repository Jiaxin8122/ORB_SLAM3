...
int static KeyPoseOptimization(KeyFrame* pKFrame){
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolver_6_3::LinearSolverType * linearSolver;
    ...
    // Set Frame vertex
    g2o::VertexSE3Expmap * vSE3 = new g2o::VertexSE3Expmap();
    vSE3->setEstimate(Converter::toSE3Quat(pKFrame->GetPose()));
    vSE3->setId(0);
    vSE3->setFixed(false);
    optimizer.addVertex(vSE3);

    // Set MapPoint vertices
    const int N = pKFrame->N;
...

    {
        ...
        for(int i=0; i<N; i++)
        {
            MapPoint* pMP = pKFrame->mvpMapPoints[i];
            if(pMP)
            {
                //Conventional SLAM
                if(!pKFrame->mpCamera2){
                    // Monocular observation
                    if(pKFrame->mvuRight[i]<0)
                    {
                        nInitialCorrespondences++;
                        pKFrame->mvbOutlier[i] = false;

                        Eigen::Matrix<double,2,1> obs;
                        const cv::KeyPoint &kpUn = pKFrame->mvKeysUn[i];
                        obs << kpUn.pt.x, kpUn.pt.y;
                        ...
                        e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(0)));
                        e->setMeasurement(obs);
                        const float invSigma2 = pKFrame->mvInvLevelSigma2[kpUn.octave];
                        e->setInformation(Eigen::Matrix2d::Identity()*invSigma2);
                    ...
                    else  // Stereo observation
                    {
                        ...
                        Eigen::Matrix<double,3,1> obs;
                        const cv::KeyPoint &kpUn = pKFrame->mvKeysUn[i];
                        const float &kp_ur = pKFrame->mvuRight[i];
                        obs << kpUn.pt.x, kpUn.pt.y, kp_ur;
                ...
                //SLAM with respect a rigid body
                    ...
                    if (i < pKFrame->NLeft) {    //Left camera observation
                        kpUn = pKFrame->mvKeys[i];
                        ...
                        kpUn = pKFrame->mvKeysRight[i - pKFrame->NLeft];
                        ...
                        e->pCamera = pKFrame->mpCamera2;
                        cv::Mat Xw = pMP->GetWorldPos();
                        e->Xw[0] = Xw.at<float>(0);
                        e->Xw[1] = Xw.at<float>(1);
                        e->Xw[2] = Xw.at<float>(2);

                        e->mTrl = Converter::toSE3Quat(pKFrame->mTrl);
    ...
    for(size_t it=0; it<4; it++)
    {
        ...
        for(size_t i=0, iend=vpEdgesMono.size(); i<iend; i++)
        {
            ...
            if(chi2>chi2Mono[it])
            {
                pKFrame->mvbOutlier[idx]=true;
                e->setLevel(1);
                nBad++;
            }
            else
            {
                pKFrame->mvbOutlier[idx]=false;
                e->setLevel(0);
            }
